...
class SystemPrompt:
	def __init__(
		...
		# Add personalization prompt if enabled
		if enable_personalization:
			personalization_prompt = self._load_personalization_prompt()
	...
	def _load_personalization_prompt(self) -> str:
		"""Load the personalization prompt template."""
		try:
			with (
				importlib.resources.files('browser_use.agent.system_prompts')
				.joinpath('prompt_personalization.md')
				.open('r', encoding='utf-8') as f
			):
				return f.read()
		except Exception:
...
class AgentMessagePrompt:
	...
	def __init__(
		...
		self.user_context: str | None = user_context
...
"""
	...
	def get_user_message(self, use_vision: bool = True) -> UserMessage:
		...
		# Add user context from memory bank if available
