...
class Agent(Generic[Context, AgentStructuredOutput]):
	...
	def __init__(
		...
		personalization_llm: BaseChatModel | None = None,
		**kwargs,
	):
		...
		self._message_manager = MessageManager(
			...
			system_message=SystemPrompt(
				...
				enable_personalization=memory_bank_path is not None,
			).get_system_message(),
		...
		# Personalization service for handling ambiguous tasks with user context
		self._personalization_service: 'PersonalizationService | None' = None
		# Trajectory log for custom format export
		self._trajectory_log: list[dict[str, Any]] = []
		if memory_bank_path is not None:
			from browser_use.personalization.service import PersonalizationService

			self._personalization_service = PersonalizationService(
				memory_bank_path=memory_bank_path,
				llm=personalization_llm or llm,
				enabled=True,
			)
			self.logger.info(f'🧠 Personalization enabled with memory bank: {memory_bank_path}')
	...
	async def step(self, step_info: AgentStepInfo | None = None) -> None:
		...
		memory_step_info: MemoryStepInfo | None = None

		try:
			# Phase 1: Prepare context and check if memory retrieval is needed
			browser_state_summary, memory_step_info = await self._prepare_context(step_info)

			if memory_step_info is not None:
				# Memory step: only record memory retrieval, no action execution
				self.logger.info(f'🧠 Step {self.state.n_steps}: Memory retrieval step - query: {memory_step_info.memory_query}')
				# Set a placeholder result for memory step
				self.state.last_result = [ActionResult(extracted_content=f'Retrieved memory for: {memory_step_info.memory_query}')]
				self.state.last_model_output = None
			else:
				# Navigation step: Get model output and execute actions
				# Phase 2: Get model output (LLM call → decide next action)
			...
			await self._finalize(browser_state_summary, memory_step_info)

	async def _prepare_context(self, step_info: AgentStepInfo | None = None) -> tuple[BrowserStateSummary, MemoryStepInfo | None]:
		"""Prepare the context for the step: browser state, action models, page actions.

		Returns:
			tuple of (BrowserStateSummary, MemoryStepInfo | None)
			- If MemoryStepInfo is not None, this is a memory-only step (no action execution)
			- If MemoryStepInfo is None, this is a navigation step (proceed with LLM action)
		"""
		...
		# Check if memory retrieval is needed (PersonalizationService)
		memory_step_info: MemoryStepInfo | None = None
		user_context = None

		if self._personalization_service is not None:
			personalization_result = await self._personalization_service.process_step(
				task=self.task,
				current_url=browser_state_summary.url,
				page_title=browser_state_summary.title or '',
				next_action_description=self.state.last_model_output.next_goal if self.state.last_model_output else None,
			)

			if personalization_result.retrieved_context:
				# Memory was retrieved - this becomes a memory-only step
				self.logger.debug(f'🧠 Memory retrieval needed for step {self.state.n_steps}')
				memory_step_info = MemoryStepInfo(
					memory_query=personalization_result.retrieval_query or '',
					retrieved_memory_indices=personalization_result.source_entries,
					memory_answer=self._personalization_service.memory_context_cache[-1]['answer'] if self._personalization_service.memory_context_cache else '',
					current_observation=personalization_result.current_observation or '',
					memory_reason=personalization_result.memory_reason or '',
				)
				# Return early - this is a memory step, no need to create state messages for LLM
				return browser_state_summary, memory_step_info
			else:
				# No new memory retrieval needed - check if we have cached context to inject
				cached_context = self._personalization_service.get_cached_context()
				if cached_context:
					user_context = cached_context

		# Navigation step: create state messages for LLM
		...
		return browser_state_summary, None
	...
	async def _finalize(self, browser_state_summary: BrowserStateSummary | None, memory_step_info: MemoryStepInfo | None = None) -> None:
		...
		if browser_state_summary:
			...
			await self._make_history_item(
				...
				memory_step_info=memory_step_info,
			)
	...
	async def _make_history_item(
		...
		# Store screenshot and get path (disabled to save disk space)
		...
		# Add to trajectory log for custom format export
		self._add_to_trajectory_log(model_output, browser_state_summary, memory_step_info)
	...
	def _add_to_trajectory_log(
		self,
		model_output: AgentOutput | None,
		browser_state_summary: BrowserStateSummary,
		memory_step_info: MemoryStepInfo | None = None,
	) -> None:
		"""Add current step info to trajectory log for custom format export.

		Two types of steps:
		1. Memory step (memory_step_info is not None):
		   - step_type: 'memory'
		   - memory_query, retrieved_memory_indices, memory_answer

		2. Navigation step (memory_step_info is None):
		...
		if memory_step_info is not None:
			# Memory step: only memory retrieval info
			step_entry['step_type'] = 'memory'
			step_entry['current_observation'] = memory_step_info.current_observation
			step_entry['memory_reason'] = memory_step_info.memory_reason
			step_entry['memory_query'] = memory_step_info.memory_query
			step_entry['retrieved_memory_indices'] = memory_step_info.retrieved_memory_indices
			step_entry['memory_answer'] = memory_step_info.memory_answer
		else:
			# Navigation step: action info
			step_entry['step_type'] = 'navigation'

			if model_output:
				# Reasoning trace
				step_entry['reason'] = model_output.next_goal or model_output.thinking or ''

				# Actions with element info
				actions_info = []
				for action in model_output.action:
					...
					action_info: dict[str, Any] = {'action': action_name}
					if isinstance(action_params, dict):
						# Include element index if present
						if 'index' in action_params:
							action_info['element_index'] = action_params['index']
						# Include other key params
						for key in ['text', 'url', 'query']:
							if key in action_params:
								action_info[key] = action_params[key]
					actions_info.append(action_info)

				step_entry['actions'] = actions_info

		self._trajectory_log.append(step_entry)

	def save_trajectory(self, file_path: str | Path | None = None) -> None:
		"""Save trajectory log in custom format.

		Each step has a 'step_type' field that is either 'memory' or 'navigation'.

		Format for memory steps (step_type='memory'):
		- step: step number
		- url: current URL
		- step_type: 'memory'
		- memory_query: query used to retrieve from memory bank
		- retrieved_memory_indices: indices of retrieved memories
		- memory_answer: generated answer from retrieved memories

		Format for navigation steps (step_type='navigation'):
		- step: step number
		- url: current URL
		- step_type: 'navigation'
		- reason: reasoning trace for action decision
		- actions: list of actions with element info
		"""
		if not file_path:
		...
		with open(file_path, 'w', encoding='utf-8') as f:
			json.dump(self._trajectory_log, f, indent=2, ensure_ascii=False)

		self.logger.info(f'📝 Trajectory saved to {file_path}')
