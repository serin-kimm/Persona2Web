...
def _random_delay_ms(min_ms: int = 80, max_ms: int = 220) -> int:
    """Return a random delay in milliseconds within [min_ms, max_ms]."""
    return random.randint(min_ms, max_ms)

def _human_sleep(min_ms: int = 80, max_ms: int = 220) -> None:
    """Sleep for a short, human-like random delay (sync)."""
    time.sleep(_random_delay_ms(min_ms, max_ms) / 1000.0)
...
def execute_mouse_click(left: float, top: float, page: Page) -> None:
    ...
    viewport_size = page.viewport_size
    assert viewport_size
    # brief human-like delay and slight hover before click
    _human_sleep()
    page.mouse.move(
        left * viewport_size["width"], top * viewport_size["height"]
    )
...
async def aexecute_mouse_click(left: float, top: float, page: APage) -> None:
    ...
    await asyncio.sleep(_random_delay_ms() / 1000.0)
...
def execute_type(keys: list[int], page: Page) -> None:
    """Send keystrokes to the focused element with human-like random delays."""
    if _id2key[keys[-1]] == "\n":
        text = "".join([_id2key[key] for key in keys[:-1]])
        # Type each character with individual random delays
        for char in text:
            page.keyboard.type(char, delay=random.randint(30, 80))
        time.sleep(random.uniform(0.5, 1.2))  # Random pause before Enter
...
def execute_playwright_click(
    ...
    # perform the action
    _human_sleep()
    # if no delay provided, don't override; otherwise allow caller
...
def execute_action(
    ...
    # Check if new tabs were created during action execution
    all_pages = browser_ctx.pages
    if len(all_pages) > 1:
        # If there are multiple tabs, check if a new one was created
        newest_page = all_pages[-1]
        if newest_page != page and newest_page.url != page.url:
            print(f"🔄 execute_action detected new tab: {newest_page.url}")
            return newest_page
